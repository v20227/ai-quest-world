...
| `adapters/deepseek` (dsh) | replay implemented (`~/.dsh/sessions/**/session.jsonl` → UARP → settlement verified on synthetic fixtures; real-log replay pending) | local session files (zstd-compressed JSONL) |
